				...
				{toggle && (
					<motion.div
						...
						<div className="w-full flex justify-between items-center h-[8vh] border-b border-[#f1f1f155] padding-x">
							<Link
								href="/"
								className="paragraph font-semibold font-NeueMontreal text-background uppercase tracking-tight">
								Soralabs
							</Link>
							<IoMdClose
							...
							{footernavbarItems.map((item) =>
								item.disabled ? (
									<span
										key={item.id}
										aria-disabled="true"
										className="text-[80px] leading-[67px] font-FoundersGrotesk uppercase font-bold tracking-[-.9] text-background/35 cursor-not-allowed select-none">
										{item.title}
									</span>
								) : (
									<Link
